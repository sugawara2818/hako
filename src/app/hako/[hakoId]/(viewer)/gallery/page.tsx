        ...
        <div className="px-6 py-6 md:px-10 md:py-8">
              ...
              <div>
                ...
                {selectedAlbumId ? (
                  <div className="flex items-center gap-2 mt-2">
                    <div className="flex items-center gap-2 py-1 px-2 bg-white/5 rounded-lg border theme-border">
                      <div className="w-4 h-4 rounded-full overflow-hidden bg-[#82d9bc] flex items-center justify-center">
                        {albums.find(a => a.id === selectedAlbumId)?.userAvatar ? (
                          <Image 
                            src={albums.find(a => a.id === selectedAlbumId).userAvatar} 
                            alt="" 
                            width={16} 
                            height={16} 
                          ...
                          <span className="text-[8px] font-black text-gray-800">
                            {(albums.find(a => a.id === selectedAlbumId)?.userName || 'U').charAt(0).toUpperCase()}
                          </span>
                        )}
                      </div>
          ...
          {!selectedAlbumId && (
            <div className="mt-8 space-y-4">
              ...
              {/* Column Selector (Local ONLY) */}
              {!selectedAlbumId && filter === 'discovery' && (
                <div className="flex items-center gap-3 bg-white/5 p-1 rounded-2xl w-fit max-w-full overflow-x-auto hide-scrollbar">
                  {[1, 2, 3, 4, 5, 6].map(num => (
                    <button
                      key={num}
                      onClick={() => saveColumns(num)}
                      className={`w-8 h-8 rounded-xl text-[10px] font-black transition-all flex items-center justify-center shrink-0 ${
                        columns === num 
                          ? 'bg-[#82d9bc] text-gray-700 shadow-lg shadow-[#82d9bc]/20' 
                          : 'text-gray-500 hover:text-white'
                      }`}
