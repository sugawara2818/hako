...
export default async function OwnerDashboardPage({
  ...
  return (
    <div className="min-h-screen theme-bg theme-text font-sans">
      {/* Sidebar/Nav Mock */}
      <nav className="fixed top-0 w-full glass border-b theme-border z-50">
        <div className="max-w-7xl mx-auto px-2 md:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-1.5 md:gap-3 min-w-0 flex-1">
            <Link href="/owner/dashboard" className="flex items-center gap-1 md:gap-2 text-sm theme-muted hover:theme-text hover:bg-white/10 transition-colors px-1.5 md:px-3 py-2 -ml-1.5 md:-ml-3 rounded-xl group shrink-0">
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0 pl-2">
            <Link 
              href={`/hako/${hako.id}`}
              className="flex items-center gap-1 md:gap-2 text-xs md:text-sm theme-muted hover:theme-text transition-colors whitespace-nowrap"
              target="_blank"
            >
              <span className="hidden sm:inline">箱を見る</span>
              <ArrowUpRight className="w-4 h-4" />
            </Link>
            <div className="h-4 w-[1px] theme-border mx-0.5 md:mx-1" />
            <form action={signOut}>
                <button type="submit" className="text-xs md:text-sm font-medium theme-muted hover:text-red-400 flex items-center gap-1 md:gap-2 transition-colors whitespace-nowrap" title="ログアウト">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 pt-24 pb-12">
        <div className="mb-8 md:mb-10 animate-fade-in">
          ...
          <p className="text-sm md:text-base theme-muted">箱の運営状況と各種設定を管理します。</p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 min-w-0">
          ...
          <div className="lg:col-span-2 space-y-6 min-w-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-fade-in" style={{ animationDelay: '0.1s' }}>
              <div className="theme-surface p-6 rounded-2xl border theme-border">
                <div className="flex items-center justify-between mb-4">
                  <span className="theme-muted font-medium text-sm">総メンバー数</span>
              ...
              <div className="theme-surface p-6 rounded-2xl border theme-border relative overflow-hidden group">
                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 to-pink-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="relative z-10 flex flex-col h-full justify-between">
                  <div>
                    <span className="theme-muted font-medium text-sm block mb-1">今月の売上 (プレビュー)</span>
          ...
          <div className="space-y-6 animate-fade-in min-w-0" style={{ animationDelay: '0.3s' }}>
            ...
            {/* Invite Card */}
            <div className="p-6 rounded-2xl border border-purple-500/30 bg-gradient-to-b from-purple-600/10 to-transparent relative overflow-hidden theme-surface shadow-sm">
              ...
              <p className="text-sm theme-muted mb-6 relative z-10 leading-relaxed">
            ...
            {/* Hako Settings */}
            <div id="settings" className="theme-surface p-6 md:p-8 rounded-3xl border theme-border shadow-md animate-fade-in" style={{ animationDelay: '0.4s' }}>
            ...
            {/* Quick Links */}
            <div className="theme-surface p-6 rounded-3xl border theme-border shadow-sm">
              <h2 className="text-sm font-black theme-muted uppercase tracking-widest mb-4">クイックリンク</h2>
              <div className="space-y-2">
                <Link href={`/hako/${hako.id}`} target="_blank" className="w-full text-left px-4 py-3 rounded-2xl hover:theme-elevated transition-colors text-sm theme-text font-medium flex items-center justify-between group">
                  <span className="truncate pr-2">パブリック表示を確認</span>
                  <ArrowUpRight className="w-4 h-4 text-gray-500 group-hover:theme-text transition-colors shrink-0" />
                </Link>
                <button className="w-full text-left px-4 py-3 rounded-2xl hover:bg-white/5 transition-colors text-sm theme-muted font-medium flex items-center justify-between group opacity-50 cursor-not-allowed">
