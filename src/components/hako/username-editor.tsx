...
export function UsernameEditor({ hakoId, currentName }: UsernameEditorProps) {
  ...
  // Focus input when keyboard appears on mobile
  useEffect(() => {
  ...
  if (isEditing) {
    return (
      <div 
        className="flex flex-col gap-2 w-full animate-in fade-in slide-in-from-top-1 duration-200"
        onClick={e => e.stopPropagation()} // Prevent sidebar from reacting to clicks inside editor
      >
        ...
        <div className="flex items-center justify-end gap-2">
           <button
            ...
            disabled={loading}
            className="px-4 py-2 text-xs font-bold text-gray-400 hover:text-white hover:bg-white/10 rounded-xl transition-all flex items-center gap-1"
          >
          ...
          <button
            onClick={handleSave}
            disabled={loading}
            className="px-4 py-2 text-xs font-bold bg-purple-600/20 text-purple-400 hover:bg-purple-600/30 rounded-xl transition-all flex items-center gap-1 border border-purple-500/20"
          >
